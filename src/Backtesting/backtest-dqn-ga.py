...
class DQNStrategy(bt.Strategy):
    ...
    def get_state(self, data):
        return np.array(self.data_processed[data._name].iloc[-self.p.state_size:][['Close', 'RSI']].values).flatten()
...
class GAStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.toolbox.register("individual", tools.initRepeat, creator.Individual, self.toolbox.attr_float, n=3)
    ...
    def evaluate(self, individual, data_name):
        rsi_buy, rsi_sell, weight = individual
        returns = []
    ...
    def next(self):
        for data in self.datas:
            current_date = data.datetime.date(0)
            rsi = self.data_processed[data._name]['RSI'].iloc[-1]
            rsi_buy, rsi_sell, _ = self.best_individuals[data._name]
...
def main():
    ...
    tickers = st.sidebar.text_input("Tickers (comma-separated)", value="SPY,AAPL,MSFT")
    ...
    if st.sidebar.button("Run Backtest"):
        ...
        st.write(f"**Max Drawdown:** {perf_summary['Max Drawdown']:.2f}%")
