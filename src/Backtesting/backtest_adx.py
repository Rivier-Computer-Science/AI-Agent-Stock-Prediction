...
class GAStrategy(bt.Strategy):
    ...
    def evaluate(self, individual):
        rsi_buy, rsi_sell, adx_threshold, weight = individual
        returns = []
        for i in range(1, len(self.data_processed)):
            rsi = self.data_processed['RSI'].iloc[i]
            adx = self.data_processed['ADX'].iloc[i] if 'ADX' in self.data_processed else self.p.adx_threshold
    ...
    def next(self):
        ...
        adx = self.data_processed['ADX'].iloc[-1] if 'ADX' in self.data_processed else self.p.adx_threshold
        rsi_buy, rsi_sell, adx_threshold, _ = self.best_individual
...
def main():
    ...
    model_type = st.sidebar.selectbox("Model Type", ["ADX", "DQN", "GA"])
    ...
    if st.sidebar.button("Run Backtest"):
        ...
        strategy_class = {'ADX': ADXStrategy, 'DQN': DQNStrategyWrapper, 'GA': GAStrategy}[model_type]
